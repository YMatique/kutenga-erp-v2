...
export default function SettingsLayout({ children }: PropsWithChildren) {
    ...
    return (
        <div className="">
            {/* Page header */}
            <div className="bg-white border border-slate-200 rounded-[4px] shadow-xs px-6 py-4 mb-6 dark:bg-zinc-900 dark:border-white/10">
                <h1 className="text-[18px] font-semibold text-slate-900 leading-tight dark:text-white">Definições</h1>
                <p className="text-sm text-slate-500 mt-0.5 dark:text-zinc-400">Gerencie o seu perfil e preferências da conta</p>
            </div>

            <div className="flex flex-col lg:flex-row gap-6">
                {/* Settings sidebar nav */}
                <aside className="w-full lg:w-52 flex-shrink-0">
                    <div className="bg-white border border-slate-200 rounded-[4px] shadow-xs overflow-hidden dark:bg-zinc-900 dark:border-white/10">
                        <nav aria-label="Definições" className="divide-y divide-slate-100">
                            {sidebarNavItems.map((item, index) => {
                                ...
                                return (
                                    ...
                                    >
                                        {item.icon && (
                                            <item.icon className={cn('h-4 w-4 flex-shrink-0', active ? 'text-[#2DB8A0]' : 'text-slate-400 dark:text-zinc-500')} />
                                        )}
                                        {item.title}
                                    </Link>
                                );
                ...
                <div className="flex-1 min-w-0">
                    <div className="bg-white border border-slate-200 rounded-[4px] shadow-xs p-6 max-w-4xl dark:bg-zinc-900 dark:border-white/10">
                        {children}
                    </div>
                </div>
            </div>
